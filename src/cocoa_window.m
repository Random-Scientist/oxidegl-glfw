...
static void updateCursorMode(_GLFWwindow *window) {
  if (window->cursorMode == GLFW_CURSOR_DISABLED) {
    ...
    _glfwGetCursorPosCocoa(window, &_glfw.ns.restoreCursorPosX,
                           &_glfw.ns.restoreCursorPosY);
    _glfwCenterCursorInContentArea(window);
    CGAssociateMouseAndMouseCursorPosition(false);
  } else if (_glfw.ns.disabledCursorWindow == window) {
    _glfw.ns.disabledCursorWindow = NULL;
    _glfwSetCursorPosCocoa(window, _glfw.ns.restoreCursorPosX,
                           _glfw.ns.restoreCursorPosY);
...
- (void)windowDidMove:(NSNotification *)notification {
  if (window->context.source == GLFW_NATIVE_CONTEXT_API)
    // LOOKHERE [window->context.nsgl.object update];
...
static GLFWbool createNativeWindow(_GLFWwindow *window,
  ...
  if (window->monitor) {
    ...
    if (wndconfig->xpos == GLFW_ANY_POSITION ||
      ...
      const int ypos =
          _glfwTransformYCocoa(wndconfig->ypos + wndconfig->height - 1);
  ...
  if (window->monitor)
    ...
    if (wndconfig->xpos == GLFW_ANY_POSITION ||
      ...
      _glfw.ns.cascadePoint = NSPointToCGPoint([window->ns.object
          cascadeTopLeftFromPoint:NSPointFromCGPoint(_glfw.ns.cascadePoint)]);
    }
  ...
  _glfwGetFramebufferSizeCocoa(window, &window->ns.fbWidth,
                               &window->ns.fbHeight);
...
void _glfwSetWindowIconCocoa(_GLFWwindow *window, int count,
...
void _glfwGetWindowPosCocoa(_GLFWwindow *window, int *xpos, int *ypos) {
  @autoreleasepool {
    ...
    if (ypos)
      *ypos = _glfwTransformYCocoa(contentRect.origin.y +
...
void _glfwSetWindowPosCocoa(_GLFWwindow *window, int x, int y) {
  @autoreleasepool {
    ...
    const NSRect contentRect = [window->ns.view frame];
    const NSRect dummyRect = NSMakeRect(
        x, _glfwTransformYCocoa(y + contentRect.size.height - 1), 0, 0);
...
void _glfwSetWindowSizeCocoa(_GLFWwindow *window, int width, int height) {
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      [window->ns.object
          setFrame:[window->ns.object frameRectForContentRect:contentRect]
           display:YES];
    }
...
void _glfwSetWindowSizeLimitsCocoa(_GLFWwindow *window, int minwidth,
...
void _glfwGetFramebufferSizeCocoa(_GLFWwindow *window, int *width,
                                  int *height) {
  @autoreleasepool {
...
void _glfwGetWindowFrameSizeCocoa(_GLFWwindow *window, int *left, int *top,
...
void _glfwGetWindowContentScaleCocoa(_GLFWwindow *window, float *xscale,
                                     float *yscale) {
  @autoreleasepool {
...
void _glfwSetWindowMonitorCocoa(_GLFWwindow *window, _GLFWmonitor *monitor,
  ...
  @autoreleasepool {
    ...
    if (window->monitor == monitor) {
      if (monitor) {
        ...
        const NSRect contentRect = NSMakeRect(
            xpos, _glfwTransformYCocoa(ypos + height - 1), width, height);
    ...
    if (window->monitor) {
      ...
      if (window->numer != GLFW_DONT_CARE && window->denom != GLFW_DONT_CARE) {
        [window->ns.object
            setContentAspectRatio:NSMakeSize(window->numer, window->denom)];
      }

      if (window->minwidth != GLFW_DONT_CARE &&
          window->minheight != GLFW_DONT_CARE) {
        [window->ns.object
            setContentMinSize:NSMakeSize(window->minwidth, window->minheight)];
      }

      if (window->maxwidth != GLFW_DONT_CARE &&
          window->maxheight != GLFW_DONT_CARE) {
        [window->ns.object
            setContentMaxSize:NSMakeSize(window->maxwidth, window->maxheight)];
      }
...
GLFWbool _glfwWindowHoveredCocoa(_GLFWwindow *window) {
  @autoreleasepool {
    ...
    return NSMouseInRect(
        point, [window->ns.object convertRectToScreen:[window->ns.view frame]],
        NO);
...
void _glfwSetWindowResizableCocoa(_GLFWwindow *window, GLFWbool enabled) {
  @autoreleasepool {
    ...
    if (enabled) {
      ...
      [window->ns.object
          setStyleMask:(styleMask & ~NSWindowStyleMaskResizable)];
      const NSWindowCollectionBehavior behavior =
          NSWindowCollectionBehaviorFullScreenNone;
      [window->ns.object setCollectionBehavior:behavior];
    }
...
void _glfwSetWindowMousePassthroughCocoa(_GLFWwindow *window,
                                         GLFWbool enabled) {
  @autoreleasepool {
...
void _glfwSetCursorPosCocoa(_GLFWwindow *window, double x, double y) {
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      const NSRect globalRect =
          [window->ns.object convertRectToScreen:localRect];
      const NSPoint globalPoint = globalRect.origin;

      CGWarpMouseCursorPosition(
          CGPointMake(globalPoint.x, _glfwTransformYCocoa(globalPoint.y)));
    }
...
const char *_glfwGetScancodeNameCocoa(int scancode) {
  @autoreleasepool {
    ...
    if (UCKeyTranslate([(NSData *)_glfw.ns.unicodeData bytes], scancode,
    ...
    CFStringRef string = CFStringCreateWithCharactersNoCopy(
        kCFAllocatorDefault, characters, characterCount, kCFAllocatorNull);
    CFStringGetCString(string, _glfw.ns.keynames[key],
                       sizeof(_glfw.ns.keynames[key]), kCFStringEncodingUTF8);
    CFRelease(string);
...
int _glfwGetKeyScancodeCocoa(int key) { return _glfw.ns.scancodes[key]; }

GLFWbool _glfwCreateCursorCocoa(_GLFWcursor *cursor, const GLFWimage *image,
                                int xhot, int yhot) {
  @autoreleasepool {
    ...
    memcpy([rep bitmapData], image -> pixels,
    ...
    cursor->ns.object =
        [[NSCursor alloc] initWithImage:native hotSpot:NSMakePoint(xhot, yhot)];
...
VkResult _glfwCreateWindowSurfaceCocoa(VkInstance instance, _GLFWwindow *window,
  ...
  @autoreleasepool {
    ...
    NSBundle *bundle = [NSBundle
        bundleWithPath:@"/System/Library/Frameworks/QuartzCore.framework"];
    if (!bundle) {
      _glfwInputError(GLFW_PLATFORM_ERROR,
